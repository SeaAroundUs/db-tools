...
select 'insert into allocation.allocation_result_global
        SELECT universal_data_id,
         ...
         where universal_data_id between ' || (g.idx+1) || ' and ' || (g.idx+200000) ||
       ' group by universal_data_id'
  from generate_series((select min(universal_data_id)-1 from allocation_result_high_seas), (select max(universal_data_id) from allocation_result_high_seas), 200000) as g(idx);
