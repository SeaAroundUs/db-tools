select 'INSERT INTO web.v_fact_data(
      ...
      WITH catch(main_area_id,year,taxon_key, data_layer_id, fishing_entity_id, gear_id, catch_type_id,reporting_status_id,sector_type_id,end_use_type_id,total_catch,unit_price) AS (
        ...
        SELECT ' || lme.lme_id || ', 
         ...
         GROUP BY c.end_use_type_id ,c.data_layer_id, c.fishing_entity_id, c.gear_id, tm.time_key, c.year, c.taxon_key, a.area_key, c.sector_type_id, c.catch_type_id, c.reporting_status_id'
  from web.lme
 order by lme_id; 
